...
//     try {
//       if (editing) {
//         const { error } = await supabase.from("scenarios").update({ title: title.trim(), content: content.trim() }).eq("id", editing.id);
//         if (error) throw error;
//       } else {
//         const { error } = await supabase.from("scenarios").insert({ title: title.trim(), content: content.trim() });
//         if (error) throw error;
//       }
//       await loadScenarios();
//       startCreate();
...
//     }
//     try {
//       const { error } = await supabase.from("scenarios").delete().eq("id", deletingId);
//       if (error) throw error;
//       // Also remove any user_scenarios pointing to this scenario to avoid orphan references (optional)
...
//             <div>
//               <h1 className="text-3xl font-extrabold text-gray-900">Scenarios</h1>
//               <p className="text-sm text-gray-500">Templates used to guide AI conversations for users.</p>
//             </div>
...
//               />
//               <button
//                 onClick={() => { startCreate(); window.scrollTo({ top: 0, behavior: "smooth" }); }}
...
//                 ) : (
//                   filtered.map((s) => (
//                     <article key={s.id} className="border rounded-lg p-4 hover:shadow-md transition transform hover:-translate-y-0.5 flex justify-between gap-4">
//                       <div>
//                         <div className="text-lg font-semibold text-gray-800">{s.title}</div>
//                         <p className="text-sm text-gray-500 mt-1 line-clamp-3">{s.content}</p>
...
//                           <button
//                             onClick={() => router.push(`/chat?demoScenarioId=${s.id}`)}
...
//                 <h2 className="text-lg font-semibold text-gray-800">{editing ? "Edit scenario" : "Create scenario"}</h2>
//                 <div className="text-sm text-gray-500">{isAdmin ? "Admin" : "Viewer"}</div>
//               </div>
...
//                   <div className="text-sm text-gray-500">Tip: include clear instructions and examples.</div>
...
//                       className={`px-4 py-2 rounded text-white text-sm ${isAdmin ? "bg-indigo-600 hover:bg-indigo-700" : "bg-gray-300 cursor-not-allowed"}`}
//                     >
...
//                 {!isAdmin && (
//                   <div className="text-xs text-yellow-600">You are not an admin — create/edit/delete actions are restricted.</div>
//                 )}
//               </div>
//             </aside>
//           </div>
...
//             <p className="text-sm text-gray-600 mb-4">This will delete the scenario and unlink it from any users. This action cannot be undone.</p>
